...
[English README](README.md) · 中文文档 · [中文 Wiki](docs/wiki/Home.zh-CN.md) · [English Wiki](docs/wiki/Home.md)

[功能特性](#功能特性) · [快速开始](#快速开始) · [配置说明](#配置说明) ·
[Docker 部署](#docker-部署) · [开发指南](#开发指南) · [贡献指南](CONTRIBUTING.md) ·
[安全策略](SECURITY.md) · [行为准则](CODE_OF_CONDUCT.md) · [开源协议](#开源协议)

操作手册见仓库内的 [GitHub Wiki 文档](docs/wiki/Home.zh-CN.md)：包含数据库选择、账号接入、
...
- 商品与订单同步、单商品单规格/多规格发布、规格图片与主图排序，以及支持逐行/默认类目、自动识别和最终兜底的表格批量铺货
...
| 商品管理 | 商品同步、手工关联、单商品单规格/多规格发布、规格图片与商品图片排序、CSV/XLSX/TSV 与可选 ZIP 批量铺货、关键词获取默认类目、逐行类目优先、自动识别与“电子资料”最终兜底、任务恢复与结果导出 |
...
付款自动发货的动作链按顺序执行（发卡 → 发货模板/文本 → 转已发货）。若其中某个**消息动作**的结果
无法确认（例如平台协议超时、发送响应缺少受理凭证、推送回显迟迟不到），系统不会盲目重试该动作，
以免向买家重复发卡；但**是否连带作废后面的状态动作**有一个明确边界：

- 仅当**剩余动作全部是 `confirm_shipment`** 这类「不会再次联系买家、且平台侧幂等」的状态动作，
  且失败属于**平台侧结果不确定**（即错误链里不含 `ErrMessageNotSent` / 动作未执行）时，才会放行
  它们，使订单状态与实际交付保持一致（避免「消息已经发给买家、订单却一直停在待发货」）；
- 其余情况一律维持**整链熔断**，不推进游标、不执行剩余动作。特别是「确定未发送」以及
  「补取卡密/恢复库存失败」这类**本地一致性失败**必须熔断：此时买家并没有收到内容，
  把订单改判为已发货只会掩盖本地不一致；
- 无论是否放行，整条运行最终都会**收口为人工核对**（`needs_review`），并保留本次动作已产生的
  发货凭证，供人工比对远端发送结果；
- 放行时游标会**原子推进**且该动作不计入 `sent_count`，因此恢复时不会重放这条不确定的消息动作；
- 把 `XIANYU_CONTINUE_AFTER_UNCERTAIN` 设为 `0` 即可恢复「任何不确定都整链熔断」的保守行为。
...
欢迎通过 [Issue 模板](.github/ISSUE_TEMPLATE/) 报告可复现的问题，或按照
[贡献指南](CONTRIBUTING.md) 通过 Pull Request 提交改进。参与项目时请遵守
[行为准则](CODE_OF_CONDUCT.md)。提交前请：
